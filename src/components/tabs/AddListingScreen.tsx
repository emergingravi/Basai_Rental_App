...
import { MapPin, Upload, Check, Info, ArrowLeft, ArrowRight, LoaderCircle } from 'lucide-react'
import { AMENITIES_LIST, Listing as UiListing, ROOM_CATEGORIES } from '../../data/appSchema'
...
export const AddListingScreen: React.FC<AddListingScreenProps> = ({ onListingCreated }) => {
  ...
  const [selectedAmenities, setSelectedAmenities] = useState<string[]>(['water', 'parking', 'wifi']);
  ...
  const handleUseCurrentLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ({ coords }) => {
        setLocation(`Lat ${coords.latitude.toFixed(5)}, Lon ${coords.longitude.toFixed(5)}`);
        setLat(coords.latitude)
        setLng(coords.longitude)
        setIsDetectingLocation(false);
      },
      () => {
        setLocation('Unable to fetch location. Please check device permissions.');
        setIsDetectingLocation(false);
      },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!owner_id) {
        // eslint-disable-next-line no-alert
        alert('Owner session not found. Please sign in again before publishing a listing.')
        return
      }
      ...
      if ((result as any)?.error) {
        throw new Error((result as any).error.message || 'Failed to create listing.')
      }
      const createdRow = Array.isArray((result as any).data) ? (result as any).data[0] : null
  ...
  if (isSubmitted) {
    return (
      <div className="p-6 text-center space-y-4 pt-12 pb-24 page-shell">
        ...
        <h2 className="text-xl font-bold font-display text-gray-900 dark:text-white">Listing submitted successfully</h2>
        <p className="text-xs text-gray-500 dark:text-gray-400 max-w-xs mx-auto">
          Your room details have been saved and the uploaded photos are ready for preview.
        </p>
        ...
        <div className="surface-card bg-gray-50 dark:bg-gray-800 p-4 rounded-xl text-left border border-gray-100 dark:border-gray-700 text-xs space-y-1.5 mx-auto max-w-sm">
          ...
          <p><strong>Rent:</strong> NPR {Number(rent).toLocaleString()}/month</p>
          <p><strong>Amenities tagged:</strong> {selectedAmenities.length} selected</p>
        </div>
      ...
      <header className="sticky top-0 z-20 bg-white/95 dark:bg-gray-900/95 backdrop-blur-md px-4 pt-4 pb-3 pr-18 border-b border-gray-100 dark:border-gray-800 transition-colors">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center space-x-2">
            ...
            <h1 className="text-base font-bold font-display text-gray-900 dark:text-white">List your property</h1>
          </div>
        ...
        <div className="flex justify-between text-[10px] text-gray-400 mt-1 font-medium">
          <span className={step >= 1 ? 'text-[#1D9E75] font-bold' : ''}>Basic Info</span>
          <span className={step >= 2 ? 'text-[#1D9E75] font-bold' : ''}>Details & Rent</span>
          <span className={step >= 3 ? 'text-[#1D9E75] font-bold' : ''}>Photos & Finish</span>
        </div>
      </header>
        ...
        {step === 1 && (
          <div className="space-y-4 animate-fade-in">
            ...
            <div>
              ...
              <p className="text-[10px] text-gray-400 mt-1">Use the map pin to fetch your current device coordinates.</p>
            </div>
          ...
          <div className="space-y-5 animate-fade-in">
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <select
                  value={roomType}
                  onChange={(e) => setRoomType(e.target.value as typeof ROOM_CATEGORIES[number])}
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-[10px] text-[#1D9E75] font-medium">Tap items below to toggle</span>
              </div>
                    ...
                    >
                      <div
                        className={`w-4 h-4 rounded mt-0.5 flex items-center justify-center border text-white transition-colors shrink-0 ${
                          isChecked ? 'bg-[#1D9E75] border-[#1D9E75]' : 'border-gray-300 dark:border-gray-600'
                        }`}
                      >
                        {isChecked && <Check className="w-3 h-3 stroke-[3]" />}
                      </div>
                      <span className="text-[11px] font-medium leading-tight select-none">{item.label}</span>
                    </button>
                  );
                })}
              </div>
          ...
          <div className="space-y-4 animate-fade-in">
            <div>
              ...
              >
                ...
                <p className="text-[10px] text-gray-400 mt-1">Take a new picture or select files from your device.</p>
                ...
                <input
                  ...
                  disabled={uploading}
                />
                {uploading && <div className="text-[11px] text-[#1D9E75] font-medium mt-2 animate-pulse">Uploading images…</div>}
              </div>
            </div>
            ...
            {photos.length > 0 && (
              <div>
                ...
                <div className="flex space-x-2 overflow-x-auto pb-1">
                  {photos.map((url, i) => (
                    <div key={i} className="relative w-16 h-16 rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700 shrink-0">
            ...
            <div className="surface-card p-3 bg-gray-50 dark:bg-gray-800 rounded-xl text-[11px] text-gray-500 dark:text-gray-400 flex items-start space-x-2">
              ...
              <span>Review your type, rent, and location before publishing the listing.</span>
            </div>

            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-gray-600 dark:text-gray-400 mb-1.5">Pick on map</label>
              <MapPicker
                lat={lat}
                lng={lng}
                onChange={(latV, lngV) => {
                  setLat(latV)
                  setLng(lngV)
                  setLocation(`Lat ${latV.toFixed(5)}, Lon ${lngV.toFixed(5)}`)
                }}
              />
              <p className="text-[10px] text-gray-400 mt-1">Drag the marker to set listing coordinates.</p>
            </div>
